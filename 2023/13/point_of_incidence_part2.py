...
# Goal: find the NEW line of reflection in the input patterns after fixing a "smudge".
# A smudge is an error in the pattern, where a symbol should be the opposite type.
...
def mirror_check(original: np.ndarray) -> int:
    ...
    for idx in range(0, l - 1):
        ...
        # check the number of differences between the sublists
        # if it is 1, the that should be the solution
        if np.sum(top_sublist != bottom_sublist) == 1:
            return idx+1
